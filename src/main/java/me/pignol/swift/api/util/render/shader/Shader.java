...
public abstract class Shader implements Globals {
    ...
    private Object2IntMap<String> uniformsMap;

    public Shader(final String fragmentShader) {
        int vertexShaderID, fragmentShaderID;

        try {
            final InputStream vertexStream = getClass().getResourceAsStream("/assets/shaders/vertex.vert");
            vertexShaderID = createShader(IOUtils.toString(vertexStream), ARBVertexShader.GL_VERTEX_SHADER_ARB);
            IOUtils.closeQuietly(vertexStream);

            final InputStream fragmentStream = getClass().getResourceAsStream("/assets/shaders/fragment/" + fragmentShader);
    ...
    public void startShader() {
        ...
        if(uniformsMap == null) {
            uniformsMap = new Object2IntOpenHashMap<>();
            setupUniforms();
        }
    ...
    public int getUniform(final String uniformName) {
        return uniformsMap.getInt(uniformName);
    }
