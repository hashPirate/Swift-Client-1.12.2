...
public class LookUpUtil implements Globals {
    ...
    public static UUID getUUIDSimple(String name) {
        ...
        if (mc.getConnection() != null) {
            List<NetworkPlayerInfo> infoMap = new ArrayList<>(mc.getConnection().getPlayerInfoMap());
            NetworkPlayerInfo profile = null;
            for (int i = 0, size = infoMap.size(); i < size; i++) {
                NetworkPlayerInfo networkPlayerInfo = infoMap.get(i);
                if (networkPlayerInfo.getGameProfile().getName().equalsIgnoreCase(name)) {
                    profile = networkPlayerInfo;
                    break;
                }
            }
    ...
    public static String getName(UUID uuid) {
        String url = "https://api.mojang.com/user/profiles/" + uuidToString(uuid) + "/names";
        try {
    ...
    public static List<String> getNameHistory(UUID id) {
        List<String> list = new ArrayList<>();

        try {
            JsonArray array = getResources(new URL("https://api.mojang.com/user/profiles/" + uuidToString(id) + "/names")).getAsJsonArray();
            for (JsonElement element : array) {
                JsonObject node = element.getAsJsonObject();
                String name = node.get("name").getAsString();
                long changedAt = node.has("changedToAt") ? node.get("changedToAt").getAsLong() : 0L;
                list.add(name + " : " + new Date(changedAt).toString());
            }
    ...
    public static String findNextPlayerName(String input) {
        if (mc.getConnection() != null) {
            ...
            NetworkPlayerInfo profile = null;
            for (int i = 0, size = infoMap.size(); i < size; i++) {
                NetworkPlayerInfo info = infoMap.get(i);
                if (startsWithIgnoreCase(info.getGameProfile().getName(), input)) {
                    profile = info;
                    break;
                }
            }
        ...
        for (String str : CACHE.keySet()) {
            if (startsWithIgnoreCase(str, input)) {
                return str;
            }
        }
    ...
    public static BiMap<String, UUID> getCache() {
        return CACHE;
    }
